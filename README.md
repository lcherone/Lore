...
| **AI-assisted candidate triage** | Turns a large proposal backlog into a focused review queue without granting AI authority | Deterministic quality checks plus bounded OpenAI batches recommend add, edit, merge, ignore, human review, or possible-policy review; guarded bulk actions still require an exact human confirmation |
| **Candidate review** | Human control over what Lore learned | Search, repository/type/recommendation filters, 60-item pages, multi-select, edit, narrow, change type, merge, approve, reject, or challenge with audit history |
...
Evidence can create a candidate, never automatic authority. Choose **Triage with AI** to separate durable guidance from ordinary completed-commit activity, duplicates, conflicts, and possible policies. Lore runs deterministic checks first and sends only ambiguous candidates to OpenAI in batches of ten using bounded evidence excerpts. The five summary cards, repository/type/recommendation filters, search, priority sorting, 60-item pages, and multi-select turn a queue of hundreds into smaller decisions.

Bulk **Add to knowledge** is available only for unchanged, durable, high-confidence recommendations with enough evidence and no contradiction. Bulk **Ignore** is limited to high-confidence non-durable activity. Wording edits, merges, conflicts, uncertain items, and anything policy-like remain individual decisions. Every action is audited, ignored evidence is retained, and AI can never create a policy or perform a bulk action by itself.
