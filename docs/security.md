...
Production deployments must provide a 32+ character `SESSION_SECRET`, HTTPS at the proxy, restricted database credentials, Redis authentication, and vault/KMS-backed storage for GitHub private keys and installation tokens. A local PAT is a bootstrap credential, not a multi-tenant credential model. The bundled login is deliberately limited to demo or loopback development; production identity-provider login and session administration remain a deployment integration, not a shipped claim.
