...
5. The shipped OpenAI adapter remains proposal-only and schema validated; usage budgets, detailed telemetry, and provider governance are still required before SaaS.
...
| P04 | Durable `JobRun` and `JobEvent`                | PostgreSQL records queued, dispatched, running, retrying, succeeded, and dead-letter states, attempts, bounded results/errors, and append-only events. The API exposes organisation-scoped activity; restart smoke proves terminal recovery. | Shipped foundation | Add cancellation/operator replay, progress percentages, stale-run reconciliation, and retention controls. |
| P05 | Transactional outbox                           | API dispatch atomically creates its `JobRun`, initial event, and outbox row; Redis failure leaves a due-retry intent and the reconciler replays it idempotently after restart. The originating repository/evidence business write is not yet in the same transaction. | Partial, P0 | Move repository-connect and source-event writes into application transactions that include the outbox; prove crash points on both sides of commit. |
...
| S03 | Loopback by default and explicit network/TLS opt-in | Every native server mode defaults to `127.0.0.1`. Compose opts its container into `0.0.0.0` explicitly while publishing the port only on host loopback.                           | Shipped foundation | Keep non-loopback and TLS/proxy review as explicit deployment work; add an external reverse-proxy acceptance test before SaaS. |
| S04 | Exact Host and Origin validation                    | Full production mode enforces exact configured Host and Origin values. Local internal health probes are the only loopback-host exception, and forged/malformed values have negative API tests. | Shipped foundation | Extend the matrix for the selected SaaS proxy/CDN and set optional `LORE_ALLOWED_HOSTS` only when it has additional public API hostnames. |
| S05 | Opaque revocable auth sessions                      | `AuthSession` stores only hashed random token material with expiry, revocation, last-seen, selected organisation, rotation, logout, and active-session management. Browser cookies contain no identity or tenant claims. | Shipped foundation | Add production device/audit context, cleanup jobs, risk-based re-authentication, and external security testing before SaaS. |
| S06 | Scoped CLI/MCP/local-node tokens                    | Durable `ApiToken` records store only hashes, bind user/organisation/read-write scopes/expiry/revocation, and are rejected for account management or when organisation MCP access is off. Local loopback mode needs no second token. | Shipped with gaps | Add repository-level scopes, token rotation UX, usage audit, and the complete negative cross-tenant matrix before SaaS. |
| S07 | CSRF for browser mutations                          | Production cookie writes require an exact allowed Origin and a valid CSRF token. Tests prove missing Origin, missing token, and the successful protected local mutation; bearer/PAT identity is evaluated separately. | Shipped foundation | Extend the negative matrix across OAuth session rotation and the selected SaaS proxy before external deployment. |
| S08 | Tenant-coherent associations                        | Membership is revalidated from the store; active organisation selection is server-side; owner/admin/member/viewer and email-bound invitations are enforced for account/team paths. Broad store conformance and composite constraints remain incomplete. | Partial, P0 | Add the negative cross-tenant store/API matrix for every related ID and installation route, then add database defence in depth. |
...
| S10 | No secrets/raw source in logs                       | Logger redaction and server-only GitHub credential loading exist. Systematic content logging tests/DLP do not.                                                                  | Partial     | Add log-capture tests, pre-ingestion secret/DLP policy, and customer-controlled quarantine before external deployment.     |
...
| ID  | Suggestion                                     | Current implementation and compatibility                                                                                                        | Status               | Action and acceptance                                                                                                                      |
| --- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| G01 | PAT and App historical import                  | Both auth modes exist; all merged PRs and PR/review/comment/commit/file collections paginate; worker credentials stay out of payloads; a shared serial pacer applies a conservative hourly budget and honours primary/secondary limit headers. | Shipped | Keep PAT local-only guidance; add durable per-page checkpoints under G04. |
...
| G03 | Immutable `SourceEvent` and evidence revisions | Evidence keeps a stable provider identity and immutable versioned snapshots. Historical import detects edited retained content, appends a revision, and re-runs extraction; webhook receipt is still the only source-event record. | Partial, P0 | Add the canonical `SourceEvent` envelope, supersession semantics, minimal raw retention, and webhook edit/replay coverage. |
...
| G10 | Ad-hoc communication evidence                 | Web/API accept authorised notes and transcripts, retain provenance, extract explicit signals, compare with approved knowledge, and create review-only candidates. Full local mode uses the configured schema-validated OpenAI adapter; demo and tests remain deterministic. | Shipped locally      | Add configurable redaction/raw-text retention and provider evaluation before SaaS; keep the human-approval invariant. |
...
| O03 | Stable readers and terminal refresh             | The Activity page polls the durable job reader every five seconds and cleans up its timer on unmount. No event stream or conditional cursor exists yet. | Partial | Add cursor/ETag or SSE after job retention semantics, then prove reconnect and unmount cleanup in component tests. |
| O04 | Real AI provider                                | OpenAI Responses API adapter ships with Zod structured output, untrusted evidence isolation, `store:false`, model/prompt provenance, timeout, and retries; demo retains the mock. | Shipped with gaps | Add tenant budgets, token/cost/latency telemetry, provider-region policy, offline evals, and kill switches before SaaS. |
...
The next reliability slice should add the shared memory/Prisma store conformance suite, lifecycle idempotency, durable job-run/outbox state, and cross-tenant negative matrix. The OpenAI adapter, scoped tokens, local settings, and recurring GitHub sync are now present; SaaS work must close their operational and governance gaps rather than adding more providers first.
