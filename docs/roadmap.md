...
| P10 | Observation and report provenance | Reports link session and context in payload and complete the Prisma session transactionally. There is no first-class `ChangeObservation`, and provenance columns are incomplete. | Partial, P0 | Add bounded observation manifest/hash and explicit report columns for observation, context revision, base/current commit, repository, organisation, and session. |
...
The next code change should be **Slice 1: store conformance plus explicit change observation/report provenance**. It reuses the current stores and session flow, closes a P0 traceability gap, and provides the transaction boundary that durable jobs and source lineage will later reference. Do not start with another provider, dashboard section, or real AI adapter.
