...
export class HttpLoreClient implements LoreClient {
  public constructor(
    ...
    private readonly token = process.env.LORE_API_TOKEN
  ) {}
  ...
  async #resolveToken(): Promise<string | undefined> {
    ...
    if (!configuredPath) {
      ...
      throw new Error(
        "Remote service mode requires LORE_API_TOKEN, LORE_API_TOKEN_FILE, or `lore connect --token-file ...`"
      );
    }
    const path = resolve(configuredPath);
    const metadata = await lstat(path);
    if (!metadata.isFile() || metadata.isSymbolicLink())
      throw new Error(`Lore API token path must be a regular non-symlink file: ${path}`);
  ...
  async abandonSession(sessionId: string, reason: string): Promise<AgentSession> {
    return this.#request(`/api/sessions/${sessionId}/abandon`, {
      method: "POST",
  ...
  async #request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    const response = await fetch(new URL(path, this.config.apiUrl), {
      ...
      headers: {
        ...
        "x-lore-organisation-id": this.config.organisationId,
    ...
    if (!response.ok) {
      const issueSummary = body.issues
        ...
        .join("; ");
      throw new Error(
        [body.message ?? `Lore API returned HTTP ${response.status}`, issueSummary]
          .filter(Boolean)
          .join(" — ")
      );
    }
