...
export class PersistentJobDispatcher implements JobDispatcher {
  public constructor(
    private readonly delegate: JobDispatcher,
    private readonly ledger: JobLedger
  ) {}

  async health(): Promise<void> {
    await this.delegate.health();
  }
  ...
  async dispatch(
    ...
    const run = await this.ledger.enqueue({
      organisationId,
    ...
    if (run.state !== "queued") return { id: run.id };
    try {
      const dispatched = await this.delegate.dispatch(
        name,
        { ...payload, loreJobRunId: run.id },
        idempotencyKey
      );
  ...
  async schedule(
    ...
    if (!this.delegate.schedule)
      throw new Error("The configured job transport does not support schedules");
    return this.delegate.schedule(name, payload, schedulerId, everyMs);
  }

  async unschedule(schedulerId: string): Promise<boolean> {
    return this.delegate.unschedule ? this.delegate.unschedule(schedulerId) : false;
  }
  ...
  async reconcile(): Promise<number> {
    ...
    for (const pending of await this.ledger.pending()) {
      try {
        const job = await this.delegate.dispatch(
          pending.name,
          { ...pending.payload, loreJobRunId: pending.runId },
